...
class HNSW {
    ...
    std::vector<int> selectNbrs(
        std::vector<std::pair<float,int>>& cands, int maxM)
    {
        std::vector<int> r;

        for (int i = 0;
             i < std::min((int)cands.size(), maxM);
             i++)
        {
...
public:
    ...
    void insert(const VectorItem& item, DistFn dist) {
        ...
        G[id] = {
            item,
            lvl,
            std::vector<std::vector<int>>(lvl + 1)
        };
        ...
        for (int lc = std::min(topLayer, lvl);
             lc >= 0;
             lc--)
        {
            auto W = searchLayer(
